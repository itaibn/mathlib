...
@[simp] lemma face_split_lower_get_same (I : box ι) (i x hx) :
  face ((split_lower I i x).get hx) i = face I i :=
begin
  simp only [face, split_lower, funext_iff, (∘)],
  exact ⟨λ _, rfl, λ j, update_noteq j.2 _ _⟩
end

@[simp] lemma face_split_upper_get_same (I : box ι) (i x hx) :
  face ((split_upper I i x).get hx) i = face I i :=
begin
  simp only [face, split_upper, funext_iff, (∘)],
  exact ⟨λ j, update_noteq j.2 _ _, λ _, rfl⟩
end

lemma face_split_lower_get_of_ne (I : box ι) {i j : ι} (h : i ≠ j) (x hx) :
  face ((split_lower I i x).get hx) j = (split_lower (face I j) ⟨i, h⟩ x).get hx :=
begin
  simp only [face, split_lower, eq_self_iff_true, true_and],
...
lemma face_split_upper_get_of_ne (I : box ι) {i j : ι} (h : i ≠ j) (x hx) :
  face ((split_upper I i x).get hx) j = (split_upper (face I j) ⟨i, h⟩ x).get hx :=
begin
  simp only [face, split_upper, eq_self_iff_true, and_true],
  erw [eq_update_iff, comp_app, subtype.coe_mk],
  refine ⟨update_same _ _ _, λ j hj, update_noteq _ _ _⟩,
  rintro rfl, simpa only [subtype.coe_eta] using hj
end
...
lemma exists_split_many_inf_eq_filter (π : prepartition I) :
  ∃ s : finset (ι × ℝ), ∀ t ⊇ s, π ⊓ (split_many I t) = (split_many I t).filter (λ J, ↑J ⊆ π.Union) :=
begin
...
@[simps] def {u v} upper_sub_lower {ι : Type u} {G : Type v} [fintype ι] [add_comm_group G]
  (I₀ : box ι) (i : ι) (f : ℝ → box (({i}ᶜ : set ι) : Type u) → G)
  (fb : Icc (I₀.lower i) (I₀.upper i) → (({i}ᶜ : set ι) : Type u) →ᵇᵃ[I₀.face i] G)
  (hf : ∀ x (hx : x ∈ Icc (I₀.lower i) (I₀.upper i)) J, f x J = fb ⟨x, hx⟩ J) :
  ι →ᵇᵃ[I₀] G :=
of_map_split_add
  (λ J : box ι, f (J.upper i) (J.face i) - f (J.lower i) (J.face i)) I₀
  begin
