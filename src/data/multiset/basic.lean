...
by { revert s,
     exact multiset.induction_on t (by simp) (λ a t IH s, by simp [IH, erase_le_iff_le_cons]) }

instance : has_ordered_sub (multiset α) := ⟨λ s t u, sub_le_iff_le_add⟩
...
@[simp] theorem add_sub_cancel_left (s t : multiset α) : s + t - s = t :=
add_sub_cancel_left

@[simp] theorem add_sub_cancel (s t : multiset α) : s + t - t = s :=
add_sub_cancel_right

theorem sub_le_sub_right (h : s ≤ t) (u : multiset α) : s - u ≤ t - u :=
sub_le_sub_right' h u

theorem sub_le_sub_left (h : s ≤ t) (u : multiset α) : u - t ≤ u - s :=
sub_le_sub_left' h u
...
theorem union_le_union_right (h : s ≤ t) (u : multiset α) : s ∪ u ≤ t ∪ u :=
...
theorem union_le_union_left (h : s ≤ t) (u : multiset α) : u ∪ s ≤ u ∪ t :=
sup_le_sup_left h _
